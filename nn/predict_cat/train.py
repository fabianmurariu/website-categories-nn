...
def create_batch(b):
    x = np.array([x for x, _ in b])
    y = np.array([y for _, y in b])
    return x, y


def get_data(features_path, batch_size):
    # valid
    ds1 = list(read_lines(data_point, features_path + '/valid.json'))
    valid_ds = [create_batch(b) for b in grouper([(x, y) for x, y, _ in ds1], batch_size)]
    ...
    train_ds = (create_batch(b) for b in grouper(lazy_train_set, batch_size))
...
def fit_model(model, valid_ds, train_ds, class_weights):
    model.fit_generator(train_ds, validation_data=itertools.cycle(valid_ds), steps_per_epoch=500,
                        validation_steps=len(valid_ds),
                        epochs=90, class_weight=class_weights)
...
if __name__ == "__main__":
    ...
    print("loading data")
    valid_ds, train_ds = get_data(features_path, batch_size)
    ...
    save_model(model, BASE_DIR + '/dmoz/model-keras')
