...
class VocabularySpec extends FlatSpec with Matchers {
  ...
  "Vocabulary" should "load words from json lines in a file" in {
    ...
    val vocab = VocabularyModule.dict(words)
    vocab("dsl") should be(17346)
    vocab("bid") should be(17340)
  }

  it should "load the text labels from the lines of a file" in {
    val labels =
      """
        |cars
        |people
        |dogs
      """.split("\\|")
    VocabularyModule.labels(labels) should contain theSameElementsAs List("cars", "people", "dogs")
  }
}
